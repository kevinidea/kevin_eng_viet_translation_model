...
class VocabEntry(object):
    ...
    def __init__(self, word2id=None):
        ...
        # Add all possible characters in both source and target languages (e.g. English and Vietnamese)
        self.char_list = list("""ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz
            0123456789,;.!?:'\"/\\|_@#$%^&*~`+-=<>()[]
            ÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚÝàáâãèéêìíòóôõùúýĂăĐđĨĩŨũƠơƯưẠạẢảẤấẦầẨẩẪẫẬậẮắẰằẲẳẴẵẶặẸẹ
            ẺẻẼẽẾếỀềỂểỄễỆệỈỉỊịỌọỎỏỐốỒồỔổỖỗỘộỚớỜờỞởỠỡỢợỤụỦủỨứỪừỬửỮữỰựỲỳỴỵỶỷỸỹ
            —©–""")
    ...
    def words2charindices(self, sents):
        ...
        for sent in sents:
            sent_idx = []
            for word in sent:
                word_idx = [self.start_of_word]
                vi_chars = []
                for char in word:
                    try:
                        char_idx = self.char2id[char]
                    except:  # unknown char is assigned to <unk> char index
                        # print(f"{char} is an unknown character")
                        char_idx = self.char2id['<unk>']
                    word_idx.append(char_idx)
                word_idx.append(self.end_of_word)
                sent_idx.append(word_idx)
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        # padded_indices has shape (batch_size, max_sentence_length, max_word_length)
        indices = self.words2charindices(sents)
        padded_indices = pad_sents_char(indices, self['<pad>'])
        # reshape indices to shape (max_sentence_length, batch_size, max_word_length)
        # difference between .view vs .permute vs .transpose
        # https://discuss.pytorch.org/t/different-between-permute-transpose-view-which-should-i-use/32916
        sents_var = torch.LongTensor(padded_indices).permute(1, 0, 2).to(device)
...
if __name__ == '__main__':
    ...
    #     return sents_idx
    #
    # sents = [['this', 'is'], ['a', 'another', 'test']]
    # print(words2charindices(sents))
    #
    # def to_input_tensor_char(sents: List[List[str]], device: torch.device) -> torch.Tensor:
    #     indices = words2charindices(sents)
    #     padded_indices = pad_sents_char(indices, 0)
    #     sents_var = torch.LongTensor(padded_indices).permute(1, 0, 2).to(device)
    #
    #     return sents_var
    # print('-' * 70)
    # print(to_input_tensor_char(sents, torch.device('cpu')))